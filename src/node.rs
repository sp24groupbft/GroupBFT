...
impl Node {
    pub(crate) fn new(config: NodeConfig, network: MemoryNetworkAdaptor, genesis: Block) -> Self {
        ...
        let group_voter_set = config.get_group_voter_set();
        let state = Arc::new(Mutex::new(Environment::new(block_tree, voter_set, group_voter_set, network)));
