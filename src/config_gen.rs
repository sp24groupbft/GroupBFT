        ...
        for (i, pair) in ret.iter().enumerate() {
            if i == 0 && self.index == 0 {
                content.push_str(&format!(
                    "./GBFT --config {} --export-path result-{}.json &\n",
                    pair.0.display(),
                    result_label
                ));
            } else {
                content.push_str(&format!(
                    "./GBFT --config {} --disable-metrics &>/dev/null &\n",
                    pair.0.display()
                ));
            }
...
impl DistributionPlan {
    pub(crate) fn new(
        ...
        //info!("voter set {:#?}", voter_set);

        let mut peer_addrs = BTreeMap::new();
        let mut chunk_addr = BTreeMap::new();
        ...
        let cvec:Vec<(_, _)> = peer_addrs.clone().into_iter().collect();
        let mut result = Vec::new();

        let chunks = cvec.chunks(per_number);

        for chunk in chunks{
          let mut chunk_map = BTreeMap::new();
          for (k,v) in chunk{
            chunk_map.insert(k.clone(), *v);
          }
          result.push(chunk_map);
        }
        ...
        voter_set
            ...
            .for_each(|(idx, (pub_key, priv_key))| {
                ...
                for chunk in result.clone(){
                  if(chunk.contains_key(&pub_key)){
                    chunk_addr = chunk;
                  }
                }
                config.set_group_peer_addrs(chunk_addr.clone());
    ...
    fn new_run_scripts(&self, hosts: &Vec<String>) -> Result<Vec<(PathBuf, String)>> {
        ...
        for host in hosts {
            ...
            content.push_str(&format!("ssh {} 'killall GBFT' &\n", host));
